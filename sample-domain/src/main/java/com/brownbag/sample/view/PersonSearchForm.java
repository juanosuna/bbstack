...
import com.brownbag.core.ui.EntitySearchForm;
...
@Scope("session")
public class PersonSearchForm extends EntitySearchForm<Person> {

    @Resource(name = "personSearchFormConfig")
    private FormConfig formConfig;

    private PersonQueryFieldFactory personQueryFieldFactory;
    ...
    @Override
    public void init() {
        setCaption("Person Search Form");
        personQueryFieldFactory = new PersonQueryFieldFactory(formConfig);
        personQueryFieldFactory.addValueChangeListener("country", this, "countryChanged");
        setFormFieldFactory(personQueryFieldFactory);
    }

    public void countryChanged(Field.ValueChangeEvent event) {
        Country newCountry = (Country) event.getProperty().getValue();
        ComboBox stateCombo = (ComboBox) personQueryFieldFactory.getField("state");
    ...
    public class PersonQueryFieldFactory extends EntityFieldFactory<PersonQuery> {
        public PersonQueryFieldFactory(FormConfig formConfig) {
            super(formConfig);
        }
